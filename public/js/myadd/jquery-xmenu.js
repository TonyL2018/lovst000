	...
	$.fn.xMenu = function(options) {
		return $(this).each(function() {
			...
			$mli.click(function(){
				...
				if($li.hasClass("current01")){
					$selectUl.find("li[rel='"+id+"']").remove();					
					$li.removeClass("current01");
				}else{					
					$("<li rel='"+id+"' class='current01'>"+val+"</li>").appendTo($selectUl);
					$li.addClass("current01");
				}				
			...
			$("li",$selectUl).live('click',function(){
				var $li = $(this);
				var id  = $li.attr("rel");
				$mli.filter("li[rel='"+id+"']").removeClass("current01");	
				$li.remove();
				if($selectUl.children("li").length==0){
					$selectinfo.hide();
				}
			...
			var setCurrentItem = function (val){				
				if(val&&val!=""){
					$mli.removeClass("current01");
					var array = val.split(",");
					$selectUl.empty();
					for(var i=0;i<array.length;i++){
						var $cli = $mli.filter("li[rel='"+array[i]+"']");
						$cli.addClass("current01");
						$("<li rel='"+array[i]+"' class='current01'>"+$cli.text()+"</li>").appendTo($selectUl);
					}					
				}else{
					$selectinfo.hide();
				}
